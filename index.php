...
<body>
    <header>
        <!--Navbar-->
        <nav class="navbar fixed-top navbar-expand-lg bg-dark shadow-sm justify-content-between">
            <div class="container-fluid">
                <h5 class="container text-left text-light"
                    style="font-family: 'Comic Sans MS', sans-serif; font-weight: bold; color: dark;">
                    Perpustakaan Sulthan<img src="assets/header.png" alt="Logo"
                        style="width: 40px; height: 40px; border-radius: 50%; margin-left: 10px;">
                </h5>
                <button class="menu-button text-light" onclick="toggleDropdownMenu()">MENU</button>
...
if (isset($_POST["bahasa"])) {
        $buku = queryReadData("SELECT * FROM buku WHERE kategori = 'bahasa'");
    }
if (isset($_POST["olahraga"])) {
    ...
    <section id="homeSection" class="p-13">
...
<button type="submit" name="bahasa" class="btn btn-light border-dark">Bahasa</button>
<button type="submit" name="olahraga" class="btn btn-light border-dark">Olahraga</button>
            </div>
        </form>
